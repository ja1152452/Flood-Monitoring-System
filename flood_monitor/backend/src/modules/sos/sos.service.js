            ...
            COALESCE(
              (
                ...
                FROM (
                  ...
                  SELECT br.id, br.assigned_responder_id AS responder_id, 'BACKUP' AS dispatch_type, 'DISPATCHED' AS status, br.created_at AS dispatched_at
                  FROM backup_requests br
                  WHERE br.sos_id = s.id AND br.assigned_responder_id IS NOT NULL AND br.status = 'DISPATCHED'
                ) d
    ...
    if (!check.length) {
      await client.query(
        `INSERT INTO sos_dispatches (sos_id, responder_id, dispatched_by, dispatch_type, status, responded_at)
         VALUES ($1, $2, $2, 'BACKUP', 'ACCEPTED', NOW())
         ON CONFLICT (sos_id, responder_id)
...
export const cancelSOS = async (userId, sosId) => {
  ...
  if (!rows.length) throw ApiError.notFound('SOS not found or cannot be cancelled');

  // Reset any assigned responders back to AVAILABLE
  await query(
    `UPDATE users SET responder_status = 'AVAILABLE'
     WHERE id IN (SELECT responder_id FROM sos_dispatches WHERE sos_id = $1)`,
    [sosId]
  );
