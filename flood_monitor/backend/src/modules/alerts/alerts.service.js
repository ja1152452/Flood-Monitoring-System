  ...
  for (const user of recipients) {
    ...
    if (user.fcm_token) {
      ...
      const LEVEL_META = {
        ...
        ALERT: { label: 'Alert' },
        EVACUATION: { label: 'Evacuation' },
        CRITICAL: { label: 'Critical' },
      };
      const meta = LEVEL_META[level] || { label: level };
      const timeText = level === 'CRITICAL'
        ? 'Immediate danger. Evacuate NOW.'
        : estimatedHours
          ? `Rising flood expected in approximately ${estimatedHours} hour${estimatedHours !== 1 ? 's' : ''}.`
          : 'Rising flood detected. Please be ready to evacuate.';
      const { rows: centers } = await db.query(
        `SELECT ec.name, (ec.capacity_total - ec.capacity_current) AS available_slots
         FROM evacuation_centers ec
         LEFT JOIN barangays b ON b.id = ec.barangay_id
         LEFT JOIN users u ON u.barangay_id = ec.barangay_id
         WHERE ec.is_open = TRUE AND ec.capacity_current < ec.capacity_total
           AND u.id = $1
         ORDER BY available_slots DESC LIMIT 5`,
        [user.id]
      );
      const centerLines = centers.length
        ? centers.map(c => `• ${c.name} (${c.available_slots} slots)`).join('\n')
        : '• No available centers in your barangay.';
      const pushBody = `${timeText} Please be ready to evacuate.\n\nAvailable evacuation centers:\n${centerLines}`;
      try {
        await sendPushNotification(user.fcm_token, `🚨 Water level has reached ${meta.label} level.`, pushBody);
      } catch (fcmError) {
...
export const getActive = async () => {
  ...
  // attach real rate_per_hour to each alert
